<script>
                function myFunction(x) {
                    ...
                    if(x.value=="non_like")
                    { 
                        x.src='images/like.png'
                        x.value="like"; 
                    }
                    else
                    {
                        x.src='images/non_like.png'
                        x.value="non_like";
                    }
			...
			console.log(this.status);
                    ...
                    xmlhttp.open("GET", "http://localhost/project/like.php?like="+x.value+"&id="+x.name, true);
                    xmlhttp.send();
                }
        </script>
<?php
...
if($SEARCH == "myprojects"){
...
else if($SEARCH == "interest"){
   	$interestFlag=1;
	$res = pg_query($db, "SELECT * FROM users WHERE username='$UNAME' limit 1"); //query for interest
    ...
    $result=pg_query($db,"SELECT * FROM projectsOwnership WHERE category='$value' AND projectStatus <> 'DELETED'");
	...
	$result = pg_query($db, "SELECT * FROM projectsOwnership  WHERE projectStatus <> 'DELETED' ORDER BY likes DESC "); 
}
...
// for getting likes from array
$likesresults= pg_query($db, "SELECT id , likeduser FROM likestable WHERE likeduser = '$UNAME'");
$likearray=array();
if($likesresults)
{
$likesresults = pg_fetch_all($likesresults);
foreach ($likesresults as $likesresult)
{
	$row = array_values($likesresult);
	$likearray[$row[0]]=true;
}
}
...
if ($myinvestmentsflag == 1) {
	...
	foreach ($projects as $project){
	   ...
	   $res = pg_query($db, "SELECT * FROM projectsownership WHERE projectid='$id' AND ownername='$ownerName' limit 1"); //query for interest
	   while ($row = pg_fetch_row($res)) {
		...
		$table_contents .= "<h3 align='right'><b>Likes:" . $numlikes . "</b></h3>";
		...
		{
		if(isset($likearray[$id .','. $ownerName]))
			$table_contents .='<input type="image"  name='. $id .','. $ownerName .' src="images/like.png" value="like" onclick="myFunction(this)" width="48" height="48">';
		else
			$table_contents .='<input type="image"  name='. $id .','. $ownerName .' src="images/non_like.png" value="non_like" onclick="myFunction(this)" width="48" height="48">';
		}
...
//	echo "amountRaised";
	...
	foreach ($projects as $project){
	   ...
	   while ($row = pg_fetch_row($res)) {
		...
		{
		if(isset($likearray[$id .','. $ownerName]))
			$table_contents .='<p><input type="image" name='. $id .','. $ownerName .' src="images/like.png" value="like" onclick="myFunction(this)" width="48" height="48"></p>';
		else
			$table_contents .='<p><input type="image" name='. $id .','. $ownerName .' src="images/non_like.png" value="non_like" onclick="myFunction(this)" width="48" height="48"></p>';
		
		}
		$table_contents .= "<span class='w3-right'>" . $category . "</span></p>";
		$table_contents .= "</div>";
	}
...
<div class="w3-row">
  ...
  <div class="w3-col m6 w3-center"><p></p>
	...
	<a href="index.php?popular=true"> Show popular projects</a>
  </div>
  <div class="w3-col m3 w3-center"><p></p></div>
</div>
